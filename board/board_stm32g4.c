...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if (GPIO_Pin == BTN_SEL_Pin){
		board_btn_sel_callback();
	}
	else if (GPIO_Pin == BTN_ENTER_Pin){
...
void board_start(){
	...
	BoardAssert(_Board.xI2Cmutex != NULL);
...
void board_eeprom_read(uint32_t i2cAddress,
		uint32_t memAddress,
		uint8_t memAddrSize,
		uint8_t *data,
		uint32_t len){
...
void board_eeprom_write(uint32_t i2cAddress,
		uint32_t memAddress,
		uint8_t memAddrSize,
		uint8_t *data,
		uint32_t len){
